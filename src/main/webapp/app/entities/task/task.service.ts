...
export class TaskService {
    ...
    protected convertDateFromClient(task: ITask): ITask {
        const copy: ITask = Object.assign({}, task, {
            assignedDate: task.assignedDate != null && task.assignedDate.isValid() ? task.assignedDate.toJSON() : null,
            dueDate: task.dueDate != null && task.dueDate.isValid() ? task.dueDate.toJSON() : null
        });
        return copy;
    }
    ...
    protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
        if (res.body) {
            res.body.assignedDate = res.body.assignedDate != null ? moment(res.body.assignedDate) : null;
    ...
    protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
        if (res.body) {
            res.body.forEach((task: ITask) => {
                task.assignedDate = task.assignedDate != null ? moment(task.assignedDate) : null;
